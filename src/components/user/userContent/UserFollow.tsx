...
import { getUserFollowList, toggleUserFollow } from "../../../api/user.api";
...
const UserFollow = ({ artistInfo }: UserFollowProps) => {
  ...
  const { data: userFollowlistData, refetch: fetchUserFollowList } =
    useQuery<UserFollowList>([userId + "userFollowList"], () =>
      getUserFollowList(String(userId))
    );
  ...
  const isFollowed = userFollowlistData?.followingInfos.some(
